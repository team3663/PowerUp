...
public class CG_AutoMidDiffSide extends CommandGroup {
	...
	public CG_AutoMidDiffSide(boolean switchRight) {
		if (switchRight) {
			addSequential(C_DriveForwardRelative.fromInches(90, .5));
			addSequential(new C_TurnRelativeAngle(60, .3));
			addSequential(C_DriveForwardRelative.fromInches(20, .25));
			...
			addSequential(C_DriveForwardRelative.fromInches(150, .4));
			addSequential(new C_TurnRelativeAngle(115, .4));
			addSequential(C_DriveForwardRelative.fromInches(50, .4));
		} else {
			addSequential(C_DriveForwardRelative.fromInches(90, .5));
			addSequential(new C_TurnRelativeAngle(-60, .3));
			addSequential(C_DriveForwardRelative.fromInches(20, .25));
