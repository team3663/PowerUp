...
public class CG_AutoMidSameSide extends CommandGroup {
	...
	public CG_AutoMidSameSide(boolean isRight) {
		if (isRight) {
			...
			addSequential(C_DriveForwardRelative.fromInches(30, .25));
			...
			addSequential(C_DriveForwardRelative.fromInches(65, .4));
			addSequential(new C_TurnRelativeAngle(-90, .4));
			addSequential(C_DriveForwardRelative.fromInches(36, .4));
		} else {
			addSequential(C_DriveForwardRelative.fromInches(90, .5));
			addSequential(new C_TurnRelativeAngle(-60, .3));
			addSequential(C_DriveForwardRelative.fromInches(30, .25));
